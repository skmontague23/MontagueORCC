...
df <- read_csv("growth_YEAR2_weights_raw.csv")
...
x <- read_csv("~/Desktop/MontagueORCC_repo/MontagueORCC/Oyster_Weight_Data/growth_phase2.1_areaMERGED.csv")
y <- read_csv("growth_YEAR2_weights_working.csv")
View(x);View(y)

colnames(x)

Year2_merged <- right_join(
  x,
  y,
  by = c("Sample_Name", "Phase_1_treat", "Phase_2_treat"),
  keep = NULL,
  na_matches = "never",
  unmatched = "drop"
)
...
## see which rows didnt match, these should all have NAs from tags falling off
unmatched_y <- anti_join(
  y,
  x,
  c("Sample_Name", "Phase_1_treat", "Phase_2_treat")
)

View(unmatched_y) # yes they all have an NA from missing tag info

#write CSV file with newly joined data
write.csv(Year2_merged, '/Users/sophiemontague/Desktop/MontagueORCC_repo/MontagueORCC/Year_2_Data/growth_YEAR2_weights_merged.csv')
